...
sudo chown ${consul_user}:${consul_group} ${consul_path}
echo "$(${consul_path} --version)"

sudo mkdir -pm 0755 ${consul_config_dir} ${consul_data_dir} ${consul_tls_dir}

sudo chown -R ${consul_user}:${consul_group} ${consul_config_dir} ${consul_data_dir} ${consul_tls_dir}
...
#echo "Give consul consul_user shell access for remote exec"
# todo - check on necessity of this
#sudo /usr/sbin/usermod --shell /bin/bash ${consul_user} >/dev/null
