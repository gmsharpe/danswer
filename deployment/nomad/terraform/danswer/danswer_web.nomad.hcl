job "danswer_web" {
  ...
  group "api_group" {
    ...
    task "api_server" {
      ...
      config {
        ...
        args = [
          "-c",
          "alembic upgrade head && echo \"Starting Danswer API Server\" && uvicorn danswer.main:app --host 0.0.0.0 --port ${NOMAD_PORT_api_port}"
        ]
      }
      ...
      env {
        ...
        REDIS_HOST = "cache.service.consul"
      }
  ...
  group "cache" {
    ...
    volume "cache_volume" {
      type      = "host"
      read_only = false
      source    = "cache"  # Define this as a path in your Nomad client config
    }
